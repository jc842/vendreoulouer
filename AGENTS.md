# Directives & Contraintes d'Environnement — vendreoulouer.com

Ce document consigne l'architecture technique, les contraintes d'environnement, les directives de sécurité et les règles éditoriales pour le site **vendreoulouer.com** (vendreoulouer) hébergé sur Cloudflare Pages.
...
| :--- | :--- |
| **Domaine Apex** | `vendreoulouer.com` |
| **URL Cloudflare Pages** | `https://vendreoulouer.pages.dev` |
| **Dépôt GitHub** | `jc842/vendreoulouer` (branche principale : `main`) |
...
| **Matomo Site ID** | `111` |
| **Matomo MTM Container** | `container_tTcIeZGX.js` (Container ID : `tTcIeZGX`) |
| **Serveur Publicitaire** | Revive Adserver (`https://ads.les4h.fr/`) |
| **Revive Affiliate ID** | `125` |
| **Zones Publicitaires** | Header (`750`), In-Content (`751`), Mobile Sticky (`752`), Large Rectangle (`749`), Half Page (`748`), Skyscraper (`753`) |
...
- Ce Layout injecte au build statique le composant `Matomo.astro` avec le conteneur MTM dédié : **`container_tTcIeZGX.js`**.
...
- Tout article doit obligatoirement comporter les 2 emplacements publicitaires intégrés dans le gabarit (`src/pages/blog/[...slug].astro`) :
  1. **Encart In-Content (300x250)** : `<ReviveSlot slot="inContent" />` positionné après les premiers paragraphes (Zone 751).
  2. **Encart Fin d'Article (336x280)** : `<ReviveSlot slot="largeRectangle" />` positionné avant les suggestions de maillage (Zone 749).
